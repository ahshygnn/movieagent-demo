...
function MobileHeader({ onGallery, onDemo }) {
  return (
    <header className="mobile-header">
      <div className="flex items-center gap-2 px-4 py-3">
        ...
        <button className="mobile-icon-button" onClick={onDemo} aria-label="案例展示" title="案例展示">📁</button>
      </div>
    </header>
  )
}
  ...
  return (
    <main className="mobile-content space-y-3">
      ...
      <section className="surface p-4 space-y-2">
        <div className="text-xs" style={{ color: COLORS.secondary }}>恢复已有任务</div>
        <div className="flex gap-2">
          <input className="field min-w-0" value={state.resumeId} onChange={(event) => actions.setResumeId(event.target.value)} placeholder="Task ID" />
          <button className="secondary-button" disabled={!state.resumeId.trim() || Boolean(state.action)} onClick={actions.resume}>加载</button>
        </div>
      </section>
...
function TaskScreen({ data, phase, progress, taskId, action, onFinal, onPlay }) {
  const cost = data.cost || {}
  return (
    <main className="mobile-content space-y-3">
      <div className="py-2"><h2 className="m-0 text-base font-bold">任务中心</h2></div>
      <section className="surface p-4 space-y-3">
        <div className="flex justify-between"><span style={{ color: COLORS.secondary }}>状态</span><b style={{ color: phase === 'done' ? COLORS.green : COLORS.accentLight }}>{taskId ? (phase === 'done' ? '已完成' : '进行中') : '尚未开始'}</b></div>
        ...
        {taskId && <div className="text-xs font-mono break-all" style={{ color: COLORS.muted }}>Task ID: {taskId}</div>}
      </section>
      <section className="surface p-4">
      ...
      <section className="surface p-4 space-y-3">
        ...
        <button className="primary-button w-full" disabled={!taskId || phase !== 'done' || Boolean(action)} onClick={data.final_video ? onPlay : onFinal}>{data.final_video ? '▶ 播放成片' : action === 'final' ? '正在合成…' : '🎬 合成已有镜头'}</button>
      </section>
    </main>
  )
}
...
export default function App() {
  ...
  const [resumeId, setResumeId] = useState(() => window.localStorage.getItem('movieagent-task-id') || '')
  ...
  const progress = taskId ? Number(data.task.progress || 0) : phase === 'rewritten' ? 20 : 0
  ...
  const startLive = async () => {
    ...
    try {
      ...
      setTaskId(result.task_id); setResumeId(result.task_id); window.localStorage.setItem('movieagent-task-id', result.task_id)
  ...
  const resume = async () => {
    ...
    setAction('resume'); setError('')
    try {
      const task = await getTask(resumeId.trim())
      const script = task.script_synopsis || scriptInput
      const characters = task.characters?.length ? task.characters : splitCharacters(charactersInput)
      setScriptInput(task.raw_script || script); setRewritten(script); setCharactersInput(characters.join('，')); setTaskId(resumeId.trim())
      applyTask(resumeId.trim(), task, script, characters); setSection('storyboard'); window.localStorage.setItem('movieagent-task-id', resumeId.trim())
    } catch (requestError) { setError(`加载任务失败：${requestError.message}`) }
    finally { setAction('') }
  }
  ...
  const startDemo = () => {
    setData(demoData); setScriptInput(demoData.story.raw_synopsis); setRewritten(demoData.story.synopsis); setCharactersInput(demoData.story.characters.map((item) => item.name).join('，')); setTaskId(''); setPhase('done'); setError(''); const first = allShots(demoData)[0]; setSelectedShotId(first?.id || ''); setSection('storyboard')
  }

  const openShot = (id) => { setSelectedShotId(id); setSection('editor') }
  const state = { data, phase, action, taskId, scriptInput, charactersInput, rewritten, resumeId }
  const actions = { setScriptInput, setCharactersInput, setResumeId, rewrite, startPlanning: startLive, resume, generateCharacters: generateCharacterRefs }

  return (
    <div className="mobile-app">
      <MobileHeader onGallery={() => setGalleryOpen(true)} onDemo={startDemo} />
      ...
      {section === 'task' && <TaskScreen data={data} phase={phase} progress={progress} taskId={taskId} action={action} onFinal={generateFinal} onPlay={() => setVideoOpen(true)} />}
